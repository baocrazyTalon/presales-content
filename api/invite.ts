...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Store invite in KV
  // Main record: invite:{token} → invite data
  await kv.set(`invite:${token}`, JSON.stringify(invite));

  // If there is an expiry, set a TTL on the KV key
  if (expiryDays) {
    await kv.expire(`invite:${token}`, expiryDays * 24 * 60 * 60);
  }

  // Index by file for listing: file_invite:{base64(filePath)}:{token}
  ...
  try {
    ...
    // Invite is already stored — log the email error but don't fail the invite
